...
**Note**: the visualization requires log file, which is generated by testing. It is necessary to run testing (`python3 train_detnet_dual.py -trs ah -tes ah --evaluate`) before visualization. You may want to set `--pic` as a small value for a shorter testing time.
