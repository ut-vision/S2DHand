...
from model.helper.resnet_helper import resnet50, conv3x3
import numpy as np
...
class detnet(nn.Module):
    ...
    def forward(self, x):
        ...
        uv, argmax, vmax = self.map_to_uv(hmap)
    ...
    def map_to_uv(hmap):
        ...
        vmax, argmax = torch.max(hmap, -1, keepdim=True)
        u = argmax // w
        v = argmax % w
        uv = torch.cat([u, v], dim=-1)

        return uv, argmax, vmax
...
class DetNet_adv(nn.Module):
    def __init__(self, stacks=1):
        ...
        self.hmap_adv = net_2d(258, 256, 1)
        self.dmap_adv = net_3d(279, 256, 1)
        self.lmap_adv = net_3d(342, 256, 1)
    ...
    def forward(self, x):
        ...
        x0 = torch.cat([features, pos_tile], dim=1)
        ...
        for _ in range(self.stacks):
            heat_map = self.hmap_0(x0)
            hmaps.append(heat_map)
            x = torch.cat([x0, heat_map], dim=1)
        ...
        for _ in range(self.stacks):
            heat_map_adv = self.hmap_adv(x0)
            hmaps_adv.append(heat_map_adv)
            x = torch.cat([x0, heat_map_adv], dim=1)

            dmap_adv = self.dmap_adv(x)
            dmaps_adv.append(dmap_adv)

            x = torch.cat([x, rearrange(dmap_adv, 'b j l h w -> b (j l) h w')], dim=1)

            lmap_adv = self.lmap_adv(x)
            lmaps_adv.append(lmap_adv)
        hmap_adv, dmap_adv, lmap_adv = hmaps_adv[-1], dmaps_adv[-1], lmaps_adv[-1]

        uv_adv, argmax_adv = self.map_to_uv(hmap_adv)

        delta_adv = self.dmap_to_delta(dmap_adv, argmax_adv)
        xyz_adv = self.lmap_to_xyz(lmap_adv, argmax_adv)
...
class DetNet(nn.Module):
    def __init__(self, stacks=1):
        ...
        self.joints = 21
        self.version = 1.0
        self.model_status = f"DetNet v{self.version} consists of\n"
        self.resnet50 = resnet50()
        self.model_status += f"1. feature encoder:\nresnet50-based, feature size={self.fsize}\n"

        self.hmap_0 = net_2d(self.fsize, self.fsize, 1)
        self.dmap_0 = net_3d(self.fsize + self.joints, self.fsize, 1)
        self.lmap_0 = net_3d(self.fsize + self.joints * 4, self.fsize, 1)
        self.stacks = stacks
        self.model_status += f"2. pose-detnet (w/o pose_tile):\nstacks={stacks}, hmap_0, dmap_0, lmap0\n"
