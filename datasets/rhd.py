...
class RHDDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if IS_DEBUG:
            self.data_split = 'val'
            print('<<<DEBUG MODE>>>: change data split to val.')
        ...
        for anno, clr_root, dep_root, mask_root \
            ...
            for i in tqdm(range(len(rawdatas))):
                ...
                if IS_DEBUG==True and i>=N_DEBUG_SAMPLES-1:
                    print("<<<DEBUG MODE>>>: loaded %d samples."%(i))
                    break
...
def main():
    data_split = 'train'
    rhd = RHDDataset(
        data_root="/home/lzh/dataset/hand/RHD_published_v2/",
